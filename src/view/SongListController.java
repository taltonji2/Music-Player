...
public class SongListController {
    ...
    boolean boolSongInList (String songTitle, String songArtist){
        for(Song s : listView.getItems())
            if(s.getSong().equalsIgnoreCase(songTitle) && s.getArtist().equalsIgnoreCase(songArtist))
            {return true;}
        return false;
    }
    ...
    //checks if string in TextField input is an integer if the TextField is not empty
